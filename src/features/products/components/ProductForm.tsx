...
export const ProductForm: React.FC<ProductFormProps> = ({ onSuccess, initialData }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    price: initialData?.price?.toFixed(2) || '0.00',
          ...
          <div className="flex items-center gap-1 bg-brand-cream/10 border border-brand-chocolate/10 rounded-md p-1 h-14">
            <button
              ...
              onClick={() => {
                ...
                if (current > 0) setFormData({ ...formData, price: (current - 1).toFixed(2) })
              }}
            ...
            <input
              ...
              onBlur={(e) => {
                if (e.target.value === '') {
                  ...
                  setFormData({ ...formData, price: parseFloat(e.target.value).toFixed(2) })
                }
              }}
            />
            <button
              ...
              onClick={() => {
                const current = parseFloat(formData.price) || 0
                setFormData({ ...formData, price: (current + 1).toFixed(2) })
              }}
