...
import type { PantryItem, PantryHistory } from '../../../shared/lib/db'
...
export const PantrySummary: React.FC<PantrySummaryProps> = ({ items, history, onRestock, onViewDetails }) => {
  ...
  const totalInvestment = (items || []).reduce((acc, item) => acc + ((item.currentStock || 0) * (item.lastPrice || 0)), 0)
  ...
  const monthlyHistory = (history || []).filter(h => 
    isWithinInterval(new Date(h.createdAt), { start: monthStart, end: monthEnd })
  )

  const monthlySpend = monthlyHistory
    .filter(h => h.type === 'Restock')
    .reduce((acc, h) => acc + h.totalValue, 0)

  const monthlyUsage = monthlyHistory
  ...
  return (
    <div className="flex flex-col gap-6 pb-10 pt-2">
      ...
      <div className="p-6 rounded-md bg-brand-chocolate text-white shadow-xl flex flex-col items-center gap-2 relative overflow-hidden">
        ...
        <h2 className="text-4xl font-display leading-none text-white">GH₵ {totalInvestment.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</h2>
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="p-4 rounded-md bg-brand-chocolate/5 border border-brand-chocolate/10 flex flex-col gap-1">
            ...
            <span className="text-lg font-display text-brand-chocolate">GH₵ {monthlySpend.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
          ...
          <div className="p-4 rounded-md bg-brand-chocolate/5 border border-brand-chocolate/10 flex flex-col gap-1">
            ...
            <span className="text-lg font-display text-brand-chocolate">GH₵ {monthlyUsage.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
