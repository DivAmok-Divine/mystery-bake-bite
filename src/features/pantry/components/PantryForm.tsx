...
export const PantryForm: React.FC<PantryFormProps> = ({ onSuccess, initialData, isRestock }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    lastPrice: initialData?.lastPrice?.toFixed(2) || '0.00',
  ...
  const hasChanges = initialData ? (
    ...
    parseFloat(formData.lastPrice).toFixed(2) !== (initialData.lastPrice || 0).toFixed(2) ||
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    else if (formData.currentStock <= formData.minStock) status = 'Low Stock'
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex items-center gap-1 bg-brand-cream/10 border border-brand-chocolate/10 rounded-md p-1 h-14">
            <button
              type="button"
              onClick={() => setFormData({ ...formData, currentStock: Math.max(0, formData.currentStock - 1) })}
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className={`flex items-center gap-1 bg-brand-cream/10 border border-brand-chocolate/10 rounded-md p-1 h-14 ${isRestock ? 'opacity-50 cursor-not-allowed' : ''}`}>
            <button
              type="button"
              disabled={isRestock}
              onClick={() => setFormData({ ...formData, minStock: Math.max(0, formData.minStock - 1) })}
              className="w-10 h-full bg-brand-chocolate/5 text-brand-chocolate rounded flex items-center justify-center"
            >
              <Minus size={16} />
            </button>
        ...
        <div className="flex items-center gap-1 bg-brand-cream/10 border border-brand-chocolate/10 rounded-md p-1 h-14">
          <button
            ...
            onClick={() => {
              ...
              if (current > 0) setFormData({ ...formData, lastPrice: (current - 1).toFixed(2) })
            }}
          ...
          <input
            ...
            onBlur={(e) => {
              if (e.target.value === '') {
                ...
                setFormData({ ...formData, lastPrice: parseFloat(e.target.value).toFixed(2) })
              }
            }}
          />
          <button
            ...
            onClick={() => {
              const current = parseFloat(formData.lastPrice) || 0
              setFormData({ ...formData, lastPrice: (current + 1).toFixed(2) })
            }}
