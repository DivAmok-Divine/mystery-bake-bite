  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {isLoading ? (
        ...
        <div className="py-20 text-center flex flex-col items-center gap-3 bg-brand-chocolate/5 rounded-md border border-dashed border-brand-chocolate/10 mx-1">
          <div className="w-16 h-16 bg-brand-chocolate/5 rounded-full flex items-center justify-center text-brand-chocolate/20">
            <ShoppingCart size={32} />
          </div>
          <p className="text-brand-chocolate/60 font-bold">Your pantry is empty</p>
          <p className="text-brand-chocolate/40 text-xs max-w-[200px]">Add your flour, sugar, and other supplies to start tracking.</p>
          <button onClick={() => setIsAddingItem(true)} className="btn-primary px-6 py-2 rounded-full text-xs mt-2">Add first item</button>
        </div>
        ...
        <div className="py-20 text-center flex flex-col items-center gap-3">
          ...
          <p className="text-brand-chocolate/60 font-bold">No ingredients found</p>
          <p className="text-brand-chocolate/40 text-xs">Try a different search or filter.</p>
        </div>
      ) : (
          ...
          {filteredItems.map((item) => (
            <div key={item.id} className="card px-4 pt-4 pb-2 flex flex-col gap-3 group relative overflow-hidden">
              ...
              <div
                ...
                style={{ width: `${Math.min(100, ((item.currentStock || 0) / ((item.minStock || 1) * 5)) * 100)}%` }}
              />
              ...
              <div className="flex items-center justify-between pt-2 border-t border-brand-chocolate/5">
                ...
                <div className="flex flex-col items-end gap-1">
                  <span className="text-[10px] font-bold text-emerald-600 leading-none">GH₵ {(item.lastPrice || 0).toFixed(2)} / {item.unit}</span>
